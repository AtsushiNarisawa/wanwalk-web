...
- **独立Gitリポジトリ**: 作業ディレクトリは `~/projects/wanwalk/wanwalk-web`
  （2026-08-13 に wanwalk 配下へ正式統合。app / admin と同じ親フォルダに並ぶが Git リポジトリは別。複製クローンは存在しない）
...
```bash
cd /Users/atsushinarisawa/projects/wanwalk/wanwalk-web && npm run build
```
...
```bash
cd /Users/atsushinarisawa/projects/wanwalk/wanwalk-web
...
```bash
cd /Users/atsushinarisawa/projects/wanwalk/wanwalk-web && git push origin main
```
...
cd /Users/atsushinarisawa/projects/wanwalk/wanwalk-web && vercel inspect
```
...
- URL 構造を変える変更は、旧 slug の 301 リダイレクトと GSC への影響を必ず確認する（`~/projects/wanwalk/wanwalk-web/CLAUDE.md` 参照）
